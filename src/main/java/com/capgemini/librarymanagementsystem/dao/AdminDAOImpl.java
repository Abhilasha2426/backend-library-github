...
public class AdminDAOImpl implements AdminDAO {
	...
	public boolean addBook(UserInfoBean bean) {
		...
		UserInfoBean ubean = manager.find(UserInfoBean.class, bean.getUserId());
		Iterator<BookInfo> itr = bean.getBookInfo().iterator();
		transcation.begin();
		try {
			if(itr.hasNext()) {
				BookInfo bb = itr.next();
				BookInfo book = new BookInfo();
				book.setUserId(bean.getUserId());
				book.setBookAuthor(bb.getBookAuthor());
				book.setBookName(bb.getBookName());
				book.setNoOfBooks(bb.getNoOfBooks());
				book.setPublisher(bb.getPublisher());
				ubean.getBookInfo().add(book);
			}
...
//		transaction.begin();
//
//		String viewDetails = "select from BooksRegistration where registrationId=:registrationId";
